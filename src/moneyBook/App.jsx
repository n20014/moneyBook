...
class MoneyEntry extends Component {
  ...
  onChangeDate(event) {
    this.setState({date: event.target.valuel})
  }
  ...
  onChange={(event) => this.onChanePayingIn(event)} /> 入金
              <input type="radio" value="off" checked={!this.state.payingIn}
  onChange={(event) => this.onChanePayingIn(event)} /> 出金
              </div>
              ...
              <div>金額: <input type="text" value={this.state.amount} onChange={(event) => this.onChangeAmount(eventa)} placeholder="1000" /> </div>
